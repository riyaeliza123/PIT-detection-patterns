...
2. When the cut-off is "detection_duration_days > 0 and count < 1500", when inspecting tags with a few hundred detections and a low detection_rate, there is a pattern where over a few days days it is stuck at one place, gets washed down and is stuck at the next location. (eg: 989.002028405138).
3. Detection rate < 0.01 is usually a stuck tag - need to investigate cases where count is < 1500.
...
1. To view detections of a tag:

```sql
select * 
from detections  
where tag_id = '989.001045836858'
